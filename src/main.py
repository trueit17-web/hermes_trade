...
class TradingBot:
    ...
    async def _on_telegram_signal(self, signal_event: dict):
        ...
        if sl is None and tp is None and not signal_event.get("parsed_take_profits"):
            # Реальный инцидент: канал прислал "Заполняю BOME Short" (без
            # единой цифры) — регулярка/LLM всё равно распознали пару и
            # сторону (short/long — простые ключевые слова), и сигнал
            # прошёл на автоисполнение с рыночным входом и дефолтным SL
            # (см. блок ниже в _execute_telegram_signal), хотя канал не дал
            # НИКАКОГО ценового ориентира — это была не структурированная
            # торговая идея, а комментарий/статус ("дозаливаюсь", "закрыл
            # часть" и т.п.), неотличимый по содержанию от настоящего
            # сигнала для простого парсера. Настоящий сигнал по тому же
            # символу с реальными тейками/стопом канала пришёл через 2
            # минуты и был отклонён как "уже есть открытая позиция" — то
            # есть мусорный вход заблокировал исполнение действительно
            # содержательного сигнала. sl/tp здесь — то, что вернул именно
            # парсер, ДО дефолтного SL-фоллбэка (settings.telegram_signals_
            # default_sl_pct) в _execute_telegram_signal — тот фоллбэк
            # рассчитан на сигнал, где канал дал хотя бы вход/тейк, но
            # забыл стоп, а не на полностью бесценовое сообщение.
            signal_event["reject_reason"] = (
                "нет ни SL, ни TP — похоже на информационное сообщение, а не структурированный сигнал"
            )
            logger.info(
                f"🚫 Сигнал по {pair} отклонён: нет ни SL, ни TP — вероятно, не сигнал, "
                f"а информационное сообщение канала"
            )
            await self._save_telegram_signal(signal_event, None, "rejected", None)
            return
        ...
        if settings.active_trading_mode != "signals":
            ...
            if pair in self.open_positions:
                ...
                # серии убытков внутри самого бота. Kill switch, ручная
                # пауза И дневной лимит убытков (execution_engine.
                # can_execute(), проверяется внутри create_order) по-прежнему
                # останавливают и эту сделку — это отдельный, более общий
                # аварийный стоп всей торговли (реальный инцидент, прод:
                # can_execute() раньше не включал daily_loss_limit_reached —
                # Telegram-сигнал открыл новую позицию через ~2 часа ПОСЛЕ
                # достижения дневного лимита убытков, см. can_execute() в
                # executor.py).
    ...
    def _compute_equity(self, cash_balance: float) -> float:
        ...
        for symbol, pos in self.open_positions.items():
            ...
            if not settings.is_paper:
                # Та же формула, что и для базы просадки при рестарте
                # (ExecutionEngine.real_position_equity): фьючерс — маржа +
                # PnL, а не полный номинал.
                exchange_pos = {**pos, **tracked.get(symbol, {})}
                equity += execution_engine.real_position_equity(exchange_pos, price)
            elif pos["side"] == "long":
                equity += pos["amount"] * price
            else:
                equity += (pos["entry_price"] - price) * pos["amount"]
        return equity
    ...
    async def _trading_iteration(self):
        ...
        # risk_manager.state.check_daily_loss_limit_reset() (в отличие от
        # сброса self.daily_pnl выше) раньше вызывался ТОЛЬКО реактивно —
        # из on_trade_closed() при закрытии сделки. Реальный инцидент
        # (прод): дневной лимит убытков сработал вечером, после чего
        # ЕГО НИКТО не сбросил всю следующую ночь и утро — ни одна
        # позиция не закрылась (все открытые уже были плоскими/близки к
        # нулю), новая полночь наступила и прошла, а can_trade() всё ещё
        # видел daily_loss_limit_reached=True и продолжал молча
        # блокировать вообще ЛЮБОЙ новый вход, пока не откроется/закроется
        # хоть одна сделка — то есть потенциально бессрочно, а не на один
        # календарный день, как задумано. Вызываем здесь на каждой
        # итерации — метод сам идемпотентен (см. его докстринг) и ничего
        # не делает, пока daily_loss_limit_reached не True.
        risk_manager.state.check_daily_loss_limit_reset()
