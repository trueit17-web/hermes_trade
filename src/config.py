...
class Settings(BaseSettings):
    ...
    # Тот же класс инцидента, что и с Groq выше: llama-3.3-70b был выведен
    # из бесплатного тарифа Cerebras (анонсированное выведение — 16 февраля
    # 2026) и на практике стал 404-ить "model_not_found". llama-4-scout-
    # 17b-16e-instruct — актуальная флагманская модель бесплатного тарифа
    # Cerebras на замену. Если и этот дефолт вернёт 404 — поправьте через
    # настройку в дашборде (тот же паттерн, что уже дважды случался выше).
