...
_TOOL = {
    ...
    "function": {
        ...
        "parameters": {
            ...
            # confidence НЕ в required: реальный инцидент (прод) — когда
            # is_signal=false, модель закономерно не выставляет уверенность
            # ("нет сигнала — нечему быть уверенным"), но Groq (в отличие от
            # Anthropic/Gemini) валидирует tool-call аргументы строго по
            # required-списку СЕРВЕРНО и отклоняет весь вызов 400 "missing
            # properties: 'confidence'" вместо того чтобы вернуть is_signal:
            # false как есть — на каждом "не сигнал" сообщении канала это
            # превращало штатный "не сигнал" в WARNING-ошибку в логах
            # (маскирует реальные проблемы в мониторинге /logs?level=WARNING).
            # Код и так трактует отсутствующий confidence консервативно как
            # 0.0 (см. .get("confidence", 0.0) ниже) — требовать поле в схеме
            # только мешало получить этот же результат без ошибки.
            "required": ["is_signal"],
        },
    },
}
