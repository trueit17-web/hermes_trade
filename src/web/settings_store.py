...
SETTINGS_SCHEMA: list[dict] = [
    ...
    {"key": "risk_max_total_notional_pct", "label": "Макс. суммарная экспозиция (%)", "group": "Риск", "type": "float",
     "description": "Суммарный потолок на % баланса, занятый ВСЕМИ открытыми позициями разом — независимо от лимита числа позиций и размера каждой по отдельности (те не ловят случай, когда много позиций умеренного размера в сумме съедают почти весь баланс). 0 — не ограничивать."},
    {"key": "risk_max_algo_open_positions", "label": "Макс. открытых позиций (алго)", "group": "Риск", "type": "int",
     "description": "Отдельный лимит числа открытых позиций именно от встроенных алго-стратегий (режим «алго»/active_trading_mode) — ДОПОЛНИТЕЛЬНО к общему лимиту выше, не вместо него. Позволяет дать Telegram-каналам и алго-стратегиям разные лимиты (например, пока по алго не накоплено достаточно статистики). 0 — не ограничивать отдельно, действует только общий лимит."},
    ...
    {"key": "telegram_signals_max_sl_pct_of_margin", "label": "Макс. убыток по SL на фьючерсах (% от маржи)", "group": "Telegram сигналы", "type": "float",
     "description": "Если SL канала (или дефолтный SL выше) на фьючерсах подразумевает убыток больше этого % от маржи при фактическом плече сигнала — SL урезается до этого предела. Защищает от SL, которые при высоком плече находятся за точкой ликвидации и никогда не сработают. 0 — не ограничивать."},
    {"key": "telegram_signals_min_sl_pct_of_margin", "label": "Мин. дистанция SL на фьючерсах (% от маржи)", "group": "Telegram сигналы", "type": "float",
     "description": "Если SL канала на фьючерсах теснее этого % от маржи при фактическом плече сигнала — SL раздвигается до этого предела. Защищает от преждевременного срабатывания SL на обычном шуме цены, не успев подтвердиться в сторону сигнала. Ожидается не больше значения выше (макс. убыток), иначе потолок сразу сведёт раздвинутый SL обратно. 0 — не раздвигать."},
    {"key": "telegram_signals_max_leverage", "label": "Макс. плечо для сигналов канала", "group": "Telegram сигналы", "type": "float",
     "description": "Если канал указал плечо больше этого значения — оно урезается до лимита (и глобальный дефолт плеча не трогается). Более низкое плечо расширяет ценовой зазор до SL — полезно, если биржа отклоняет SL-ордер как слишком близкий к цене при высоком плече канала. 0 — не ограничивать."},
    {"key": "telegram_risk_sizing_max_position_pct", "label": "Потолок объёма позиции при размере от риска (% баланса)", "group": "Telegram сигналы", "type": "float",
     "description": "Для каналов с заданным «Риском на сделку»: объём позиции = риск / расстояние до SL. При очень близком SL это дало бы огромную позицию — объём ограничивается этим % баланса (тогда фактический риск меньше заданного)."},
    {"key": "telegram_signals_max_age_seconds", "label": "Макс. возраст сигнала (сек)", "group": "Telegram сигналы", "type": "float",
     "description": "Если с момента сообщения канала в Telegram до момента обработки сигнала прошло больше этого времени — сигнал отклоняется: цена могла уже уйти от контекста, в котором канал давал вход. Особенно актуально, если LLM-фолбэк парсинга временно недоступен и обработка занимает дольше обычного. 0 — не проверять."},
    ...
    {"key": "market_type", "label": "Тип рынка (real-режим)", "group": "Биржи", "type": "select", "options": ["spot", "futures"],
     "description": "spot — обычный спот-рынок (шорт не поддерживается). futures — USDT-перпетуалы (linear swap), "
                     ...
                     "Тумблер задаёт рынок только по умолчанию для новых позиций; уже открытые позиции обоих "
                     "рынков ведутся независимо от текущего положения тумблера. "
                     "Тот же переключатель есть в шапке дашборда."},
...
_RISK_PARAM_MAP = {
    ...
    "risk_max_total_notional_pct": "max_total_notional_pct",
}
