...
class ExecutionEngine:
        ...
        **_ignored,
        ...
        async with get_session() as session:
            ...
            trade = Trade(
                ...
                pnl=pnl,
                pnl_pct=pnl_pct,
                leverage=pos.get("leverage") if pos is not None else None,
                holding_seconds=holding_seconds,
                outcome=outcome,
                is_open=False,
                closed_at=utcnow(),
    ...
    async def _reconcile_futures_position(self, symbol: str, pos: dict) -> None:
        ...
        if actual_amount > tracked_amount:
            ...
            # оставался полностью невидимым для бота — без SL, без учёта
            # в risk/PnL — до тех пор, пока кто-то не заметил бы вручную
            # (подтверждено на проде: 23660 контрактов на бирже против
            # 11810 отслеживаемых).
            #
            # Подхватываем излишек как источник истины: exchange["entryPrice"]
            # (ccxt/bybit) — это УЖЕ средневзвешенная цена ВСЕЙ позиции по
            # данным биржи, пересчитывать её вручную из отдельных сделок не
            # нужно (и не из чего — вторая, "фантомная" сделка никогда не
            # была видна боту). Расширяем tracked amount до фактического и
            # переставляем биржевой SL на полный объём — старый был выставлен
            # только на ранее отслеживаемую часть, оставляя разницу без
            # защиты.
            old_amount = tracked_amount
            old_entry_price = pos.get("entry_price")
            pos["amount"] = actual_amount
            new_entry_price = position.get("entryPrice")
            if isinstance(new_entry_price, (int, float)) and new_entry_price > 0:
                pos["entry_price"] = new_entry_price
            logger.warning(
                f"⚠️ Периодическая сверка фьючерсных позиций: {symbol} — на бирже открыто "
                f"{actual_amount:.8f} контрактов, учитывалось {old_amount:.8f} (вероятно, ордер, не "
                f"подтверждённый как исполненный при открытии, всё же исполнился) — объём "
                f"скорректирован до фактического, entry_price {old_entry_price} -> {pos['entry_price']}."
            )
